...
def sentence1() -> Expr:
    ...
    # Return a list of conjoined expressions
...
def sentence3() -> Expr:
    ...
    # Construct the expressions
    Expr1 = PacmanAlive_1 % (disjoin(conjoin(PacmanAlive_0, ~PacmanKilled_0),conjoin(~PacmanAlive_0, PacmanBorn_0)))
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    return {dummyExpr('a'): True}    #returns a dictionary in the form {a: True}
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    # explained with an example: if literals= [A,B,C], the A|B|C sentence is True if at least one expression is True
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combinations = list(itertools.combinations(literals,2)) # all combinations of 2 items from the literals list
    sentences = [] # list to store sentences representing the logic that at most one of the literals is true

    # Iterate through each combination of literals
    for literal in combinations:
        # Create a sentence representing the logic that at most one of the literals is true 
        # and append it to the list of sentences
        sentence = ~literal[0] | ~literal[1]
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    # We can use the previously defined functions atLeastOne and atMostOne
    # if both of these are true we will have the desired result of exactlyOne
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # for all (x, y) in all_coords:
    # If a wall is at (x, y) --> Pacman is not at (x, y)
    # Constructing a list where we store the expression for walls and pacman (using list comprehension)
    t_list = [PropSymbolExpr(wall_str, x, y) >> ~(PropSymbolExpr(pacman_str, x, y, time=t)) for (x,y) in all_coords]

    # conjoin and append them to the pacphysics_sentences list
    pacphysics_sentences.append(conjoin(t_list))

    # Pacman is at exactly one of the squares at timestep t.
    # We use a list where we append all the states (x,y) where pacman can be
    # we can use the previously defined function to select exactly one state
    # and append it to the pacphysics_sentences list

    t2_list= [PropSymbolExpr(pacman_str, x, y, time=t) for (x,y) in non_outer_wall_coords]

    pacphysics_sentences.append(exactlyOne(t2_list))

    # Pacman takes exactly one action at timestep t.
    # we can use the same reasoning as before

    t3_list = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
    
    pacphysics_sentences.append(exactlyOne(t3_list))

    # Results of calling sensorModel(...), unless None.
    # Check if it is not None
    # Then append the result of the sensorModel with arguments the ones indicated in the outline of the project.
    if sensorModel is not None:
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))
    
    # Results of calling successorAxioms(...), describing how Pacman can end in various
    # locations on this time step. Consider edge cases. Don't call if None.
    # Check if it is not none and append the results as we did before
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # We add to the KB pacphysics_axioms(...) with the appropriate timesteps
    ...
    # Make two models with findModel
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # This is similar to pacphysicsAxioms, but don’t use that method since we are using non_wall_coords
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # Is there a satisfying assignment for the variables given the knowledge base so far? 
        # Goal assertion sentence must be true if and only if all of the food have been eaten. 
        # This happens when all F ood[x, y]_t are false.
        # Use findModel and pass in the Goal Assertion and KB.
        ...
        # Add a food successor axiom: what is the relation between Food[x, y]_t + 1 and F ood[x, y]_t and Pacman[x, y]_t? 
...
def localization(problem, agent) -> Generator:
    ...
    KB += [~PropSymbolExpr(wall_str, x, y) for (x, y) in all_coords if (x, y) not in walls_list]
    
    for t in range(agent.num_timesteps):
        
        # Add pacphysics, action, and percept information to KB.
        # The previous are created based on those informations:
        # Add to KB: pacphysics_axioms(...), which you wrote in q3. 
        # Use sensorAxioms and allLegalSuccessorAxioms for localization and mapping, and SLAMSensorAxioms and SLAMSuccessorAxioms for SLAM only.
        ...
        # for localization and mapping, or numAdjWallsPerceptRules(...) for SLAM. Add the resulting percept_rules to KB.
        ...
        # This one is created based on those informations: 
        ...
        for (x, y) in non_outer_wall_coords:
            ...
            if entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time = t)) == entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x, y, time = t)):
                possible_locations.append((x, y))
            elif entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time = t)) != False:
...
def mapping(problem, agent) -> Generator:
    ...
    # Create the expressions and append them.
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x, y) in non_outer_wall_coords:
            ...
            if entails(conjoin(KB), PropSymbolExpr(wall_str, x, y)) == entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y)):
               known_map[x][y] = -1
            elif entails(conjoin(KB), PropSymbolExpr(wall_str, x, y)) != False:
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        # Add pacphysics, action, and percept information to KB.
        # UseSLAMSensorAxioms, SLAMSuccessorAxioms, and numAdjWallsPerceptRules.        
